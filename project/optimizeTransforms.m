...
subject to 
    ...
    % Smoothness
    for k = 1:n - 3 
        B_t = [ps(k, 3) ps(k, 5) 0; ps(k, 4) ps(k, 6) 0; ps(k, 1) ps(k, 2) 1];
        B_t1 = [ps(k + 1, 3) ps(k + 1, 5) 0; ps(k + 1, 4) ps(k + 1, 6) 0; ps(k + 1, 1) ps(k + 1, 2) 1];
        B_t2 = [ps(k + 2, 3) ps(k + 2, 5) 0; ps(k + 2, 4) ps(k + 2, 6) 0; ps(k + 2, 1) ps(k + 2, 2) 1];
        B_t3 = [ps(k + 3, 3) ps(k + 3, 5) 0; ps(k + 3, 4) ps(k + 3, 6) 0; ps(k + 3, 1) ps(k + 3, 2) 1];
    ...
    for i = 1:4
        for k = 1:n
            0 <= [1 0 crop_points(i, 1) crop_points(i, 2) 0 0] * ps(k, :)' <= im_size(2);
            0 <= [0 1 0 0 crop_points(i, 1) crop_points(i, 2)] * ps(k, :)' <= im_size(1);
        end
    end
